...
class Pokemons:
    ...
    def __repr__(self):
        return f'Pokemon: value: {self.value}, type: {self.type}, pos: x-{self.pos.x}, y-{self.pos.y}'
    ...
    def cal_edges(self, g: DiGraph):
        ...
        for src in g.nodes.keys():
            ...
            for dest in g.edges[g.nodes[src].id]:
                ...
                pokdis1 = math.sqrt(pow(g.nodes[src].pos.x - po.pos.x, 2) + pow(g.nodes[src].pos.y - po.pos.y, 2))
                pokdis2 = math.sqrt(pow(g.nodes[dest].pos.x - po.pos.x, 2) + pow(g.nodes[dest].pos.y - po.pos.y, 2))
                ...
                if abs(edgedis - pokdis) <= epsilon:
                    if po.type < 0:
                        ...
                        po.src = min(src, dest)
                    po.dis = math.sqrt(
                        pow(g.nodes[po.src].pos.x - po.pos.x, 2) + pow(g.nodes[po.src].pos.y - po.pos.y, 2))
